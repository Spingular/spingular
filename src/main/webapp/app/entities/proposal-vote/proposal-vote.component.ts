...
export class ProposalVoteComponent implements OnInit, OnDestroy {
  ...
  transition() {
    this.router.navigate(['/proposal-vote'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IProposalVote) {
    return item.id;
  }

  registerChangeInProposalVotes() {
    this.eventSubscriber = this.eventManager.subscribe('proposalVoteListModification', response => this.loadAll());
  }
  ...
  protected paginateProposalVotes(data: IProposalVote[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.proposalVotes = data;
  }
}
